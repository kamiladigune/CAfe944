...
import javafx.scene.layout.GridPane; // Keep if used in FXML
import javafx.scene.layout.HBox; // Keep if used in FXML
...
public class SetDailySpecialScreen implements Main.NeedsMainApp {
    ...
    @FXML private ToggleGroup choiceToggleGroup; // Define fx:id in FXML
    ...
    @FXML private GridPane newSpecialPane; // Container for new fields
    @FXML private HBox existingBox; // Container for combo box
    ...
    public void initialize() {
        ...
        // Assume ToggleGroup and Radios exist in FXML
        if (choiceToggleGroup != null) {
            setupControlToggling(choiceToggleGroup);
            // Set initial state based on default selection in FXML/ToggleGroup
        ...
        // Button action linked via FXML onAction="#handleSetSpecial"
    }

    private void configureComboBox() {
        existingItemsComboBox.setPromptText("Choose an item...");
        existingItemsComboBox.setPrefWidth(300); // Set size if needed
    ...
    private void loadMenuItems() {
        ...
        try {
            ...
            List<Item> nonSpecials = items.stream()
                .filter(item -> !item.isDailySpecial()) // Assumes method exists
    ...
    private void handleSetSpecial() {
        ...
        try {
            if (selectExistingRadio.isSelected()) {
                ...
                desc = (desc == null) ? "" : desc.trim(); // Default empty
            ...
            clearInputFields();
            // Optionally, refresh the calling screen or close this one
