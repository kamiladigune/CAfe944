...
public class Main extends Application {
    ...
    static {
        try {
            ...
            LOGGER.log(Level.CONFIG, "Logger initialised.");
        } catch (Exception e) {
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            loadScene("/com/cafe94/gui/LoginScreen.fxml");
    ...
    public Object openWindow(String fxmlPath, String title, boolean modal)
        ...
        newStage.show();
        LOGGER.info("Opened window: " + fxmlPath + " Title: " + title);
        return controller;
    }
    ...
    FXMLLoader createLoader(String fxmlPath) throws IOException {
        ...
        if (fxmlUrl == null) {
            LOGGER.severe("Cannot find FXML resource: " + fxmlPath);
            throw new IOException("Cannot load FXML: " + fxmlPath);
        }
        LOGGER.config("Loading FXML from: " + fxmlUrl);
        return new FXMLLoader(fxmlUrl);
    }

    // --- Corrected Dependency Injection Method ---
    void injectDependencies(Object controller) {
        LOGGER.fine("Injecting dependencies into: " +
                    controller.getClass().getName());
        User currentUser = sessionManager.getCurrentUser(); // Get once

        if (controller instanceof NeedsMainApp) {
             ((NeedsMainApp) controller).setMainApp(this);
        }
        if (controller instanceof NeedsSessionManager) {
             ((NeedsSessionManager) controller).setSessionManager(sessionManager);
        }

        // --- Injection based on Controller Type ---
        ...
        if (controller instanceof LoginController) {
            ...
            // ** CORRECTED INJECTION HERE **
            c.setBookingService(bookingService); // Uses IBookingService
            // Note: currentUser usually set manually from ViewCustomersScreen
            ...
            c.setMenuService(menuService);
             ...
             c.setOrderService(orderService); // Uses IOrderService
              // Note: currentUser usually set manually from ViewCustomersScreen
             ...
             // Injected via interfaces; profiles list set manually
        ...
        // --- Add more 'else if' blocks for any other controllers ---
        else {
            LOGGER.finer("No specific DI configured for type: " +
                         controller.getClass().getName());
        }
    }
