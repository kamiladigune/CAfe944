...
public class ProfileChoiceScreen implements Main.NeedsMainApp {
    ...
    private void navigateToStaffLogin() {
        System.out.println("Staff button clicked. Navigating...");
        if (mainApp == null) {
    ...
    private void navigateToCustomerLogin() {
        System.out.println("Customer button clicked. Navigating...");
         if (mainApp == null) {
             showAlert(Alert.AlertType.ERROR, "Error", "Nav error.");
             return;
         }
        try {
            mainApp.loadScene("/com/cafe94/gui/MenuViewScreen.fxml");
             Stage stage = (Stage) mainApp.primaryStage.getScene().getWindow();
             stage.setTitle("Cafe94 - Menu");
        } catch (IOException e) {
            showAlert(Alert.AlertType.ERROR, "Navigation Error",
                      "Could not open customer screen.");
        }
    }
