...
import javafx.scene.layout.VBox; // If special VBox used in FXML
...
public class TakeawayOrderScreen implements Main.NeedsMainApp {
    ...
    public void initialize() {
        ...
        configureSpecialDisplayBox(); // Setup special display area
        setupMenuTable();
        ...
        // Button action linked via FXML onAction="#handlePlaceOrder"
    }
    ...
    private void loadAndDisplaySpecial() {
        ...
        try {
            ...
            if (specials.isEmpty()) {
                ...
                System.out.println("Displayed " + specials.size() + " specials.");
            }
        } catch (Exception e) {
    ...
    private void handlePlaceOrder() {
        ...
        try {
            pickupTime = LocalTime.parse(pickupTimeField.getText(), TIME_PARSER);
        ...
        try {
            ...
            if (order != null) {
                showAlert(Alert.AlertType.INFORMATION, "Order Placed",
                    ...
                    ") placed for pick-up at " + pickupTime.format(TIME_PARSER) + ".");
                clearForm();
            } else {
