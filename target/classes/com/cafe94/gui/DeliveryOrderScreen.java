...
import javafx.scene.layout.VBox; // Keep if needed for special box
...
public class DeliveryOrderScreen implements Main.NeedsMainApp {
    ...
    public static class OrderItem {
        ...
        public OrderItem(Item item, int qty) {
             this.domainItem = new SimpleObjectProperty<>(Objects
             .requireNonNull(item));
    ...
    @FXML private Label specialDetailsLabel; // For daily special text
    @FXML private VBox specialDisplayBox; // Container for special
    @FXML private Button placeOrderButton;
    // Add other labels/controls if defined in FXML
    ...
    public void initialize() {
        ...
        System.out.println("Initializing DeliveryOrderScreen for " +
                           currentUser.getEmail());

        // Setup UI components
        setupMenuTable(); // Includes adding the button column
        ...
        deliveryAddressArea.setPromptText("Enter full delivery address...");
        ...
        // Button action linked via FXML onAction="#handleRequestOrder"
    }
    ...
    private void addItemToOrder(Item itemToAdd) {
        ...
        if (existing.isPresent()) {
            ...
            orderSummaryList.refresh(); // Refresh to show updated quantity/total
        } else {
            currentOrderItems.add(new OrderItem(itemToAdd, 1));
        }
        // Total price updates via listener
    }
    ...
    private void handleRequestOrder() {
        ...
        String address = deliveryAddressArea.getText();
        if (address == null || address.trim().isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Address Missing",
                      "Please enter a delivery address.");
            return;
        }

        // Create list of domain Items, respecting quantity
        ...
        try {
            System.out.println("Requesting delivery order via service...");
            Delivery order = orderService.placeDeliveryOrder(
                itemsToOrder, customerId, address, null); // null for notes

            if (order != null) {
                showAlert(Alert.AlertType.INFORMATION, "Order Request Sent",
                    "Delivery order (ID: " + order.getOrderID() +
                    ") sent for approval.");
                clearForm();
            } else {
                showAlert(Alert.AlertType.ERROR, "Request Failed",
                          "Could not send delivery order request.");
            }
        } catch (Exception ex) {
            handleLoadingError("request delivery", ex);
            ex.printStackTrace(); // Log stack trace for debugging
        }
    }

    private void clearForm() {
        currentOrderItems.clear(); // This triggers total update via listener
